...
function createABI(ctx, name) {
    ...
    for (let f of contract.functions.values()) {
        if (f.isGetter) {
            getters.push({
                name: f.name,
                arguments: f.args.map((v) => ({ name: v.name, type: (0, resolveABITypeRef_1.createABITypeRefFromTypeRef)(v.type) })),
                returnType: f.returns.kind !== 'void' ? (0, resolveABITypeRef_1.createABITypeRefFromTypeRef)(f.returns) : null
            });
        }
    }
    // Errors
    ...
    let codeErrors = (0, resolveStrings_1.getAllErrors)(ctx);
