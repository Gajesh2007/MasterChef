...
function writeAccessors(type, ctx) {
    // Getters
    for (let f of type.fields) {
        ctx.fun(`__gen_${type.name}_get_${f.name}`, () => {
            ctx.append(`_ __gen_${type.name}_get_${f.name}(${(0, resolveFuncType_1.resolveFuncType)(type, ctx)} v) inline {`);
            ctx.inIndent(() => {
    ...
    ctx.fun(`__gen_${type.name}_unpack`, () => {
        ctx.append(`(${(0, resolveFuncType_1.resolveFuncType)(type, ctx)}) __gen_${type.name}_unpack(${(0, resolveFuncType_1.resolveFuncType)(type, ctx)} v) asm "NOP";`);
    });
    // Not null
    ctx.fun(`__gen_${type.name}_not_null`, () => {
        ctx.append(`(${(0, resolveFuncType_1.resolveFuncType)(type, ctx)}) __gen_${type.name}_not_null(tuple v) inline {`);
        ctx.inIndent(() => {
    ...
    ctx.fun(`__gen_${type.name}_as_optional`, () => {
        ctx.append(`tuple __gen_${type.name}_as_optional((${(0, resolveFuncType_1.resolveFuncType)(type, ctx)}) v) inline {`);
        ctx.inIndent(() => {
    ...
    ctx.fun(`__gen_${type.name}_to_tuple`, () => {
        ctx.append(`tuple __gen_${type.name}_to_tuple((${(0, resolveFuncType_1.resolveFuncType)(type, ctx)}) v) inline {`);
        ctx.inIndent(() => {
            ctx.append(`var (${type.fields.map((v) => `v'${v.name}`).join(', ')}) = v;`);
            let vars = [];
            for (let f of type.fields) {
    ...
    ctx.fun(`__gen_${type.name}_from_tuple`, () => {
        ctx.append(`(${type.fields.map((v) => (0, resolveFuncType_1.resolveFuncType)(v.type, ctx)).join(', ')}) __gen_${type.name}_from_tuple(tuple v) inline {`);
        ctx.inIndent(() => {
            // Resolve vars
            let vars = [];
            let out = [];
    ...
    ctx.fun(`__gen_${type.name}_to_external`, () => {
        ctx.append(`(${type.fields.map((v) => (0, resolveFuncTupledType_1.resolveFuncTupledType)(v.type, ctx)).join(', ')}) __gen_${type.name}_to_external((${(0, resolveFuncType_1.resolveFuncType)(type, ctx)}) v) inline {`);
        ctx.inIndent(() => {
            ctx.append(`var (${type.fields.map((v) => `v'${v.name}`).join(', ')}) = v; `);
            let vars = [];
            for (let f of type.fields) {
