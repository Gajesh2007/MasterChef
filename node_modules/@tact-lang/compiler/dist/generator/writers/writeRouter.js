...
function writeRouter(type, ctx) {
    ctx.append(`(${(0, resolveFuncType_1.resolveFuncType)(type, ctx)}, int) __gen_router_${type.name}(${(0, resolveFuncType_1.resolveFuncType)(type, ctx)} self, int msg_bounced, slice in_msg) impure inline_ref {`);
    ctx.inIndent(() => {
        ...
        ctx.inIndent(() => {
            ...
            if (bouncedHandler) {
                ...
                ctx.append(`self~${(0, id_1.fn)(`__gen_${type.name}_receive_bounced`)}(in_msg);`);
                // Exit
        ...
        for (const f of type.receivers) {
            ...
            if (selector.kind === 'internal-binary') {
                ...
                ctx.inIndent(() => {
                    ...
                    ctx.append(`self~${(0, id_1.fn)(`__gen_${type.name}_receive_${selector.type}`)}(msg);`);
                    // Exit
                    ctx.append('return (self, true);');
                });
                ctx.append(`}`);
            ...
            if (selector.kind === 'internal-empty') {
                ...
                ctx.inIndent(() => {
                    ...
                    ctx.append(`self~${(0, id_1.fn)(`__gen_${type.name}_receive`)}();`);
                    // Exit
                    ctx.append('return (self, true);');
                });
                ctx.append(`}`);
            ...
            ctx.inIndent(() => {
                    ...
                    for (const r of type.receivers) {
                        ...
                        if (selector.kind === 'internal-comment') {
                            ...
                            ctx.inIndent(() => {
                                ...
                                ctx.append(`self~${(0, id_1.fn)(`__gen_${type.name}_receive_comment_${hash}`)}();`);
                                // Exit
                                ctx.append('return (self, true);');
                            });
                            ctx.append(`}`);
                ...
                if (fallback) {
                    ...
                    ctx.inIndent(() => {
                        ...
                        ctx.append(`self~${(0, id_1.fn)(`__gen_${type.name}_receive_comment`)}(in_msg.skip_bits(32));`);
                        // Exit
                        ctx.append('return (self, true);');
                    });
                    ctx.append(`}`);
        ...
        if (fallbackReceiver) {
            ...
            ctx.append(`self~${(0, id_1.fn)(`__gen_${type.name}_receive_fallback`)}(in_msg);`);
            ctx.append('return (self, true);');
        }
        else {
            ctx.append();
