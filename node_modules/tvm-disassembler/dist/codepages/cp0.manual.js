...
CP0Manual.insertHex('8D', 8, (slice) => {
    ...
    let dataLen = 8 * len + 6;
    let x = slice.loadUintBig(dataLen);
    return `${r} ${len} ${x.toString(16)} PUSHSLICE`;
});
CP0Manual.insertHex('8E', 7, (slice, indent) => {
    let args = slice.loadUint(9);
    let refs = (args >> 7) & 3;
    let dataBits = (args & 127) * 8;
    return `<{\n${(0, disassembler_1.decompile)(slice.loadRef().beginParse(), indent + 2)}${new Array(indent).fill(' ').join('')}}> PUSHCONT`;
});
CP0Manual.insertHex('9', 4, (slice, indent) => {
    let len = slice.loadUint(4);
...
CP0Manual.insertBin('111101001010', (slice) => {
    let something = slice.loadBit();
    let something2 = slice.loadBit();
    let n = slice.loadUint(10);
